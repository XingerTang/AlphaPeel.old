...
def fill(ind, peelingInfo, phase):
    if phase:
        updatePenetrance_phase(
            ind.haplotypes[0], ind.haplotypes[1], peelingInfo.penetrance[ind.idn, :, :]
        )
    else:
        updatePenetrance_no_phase(
            ind.haplotypes[0], ind.haplotypes[1], peelingInfo.penetrance[ind.idn, :, :]
        )
...
def updatePenetrance_phase(hap0, hap1, penetrance):
    ...
    for i in range(nLoci):
        penetrance[:, i] = [0.25, 0.25, 0.25, 0.25]
...
def main():
    ...
    InputOutput.writeIdnIndexedMatrix(
        pedigree, peelingInfo.recomb, args.out + ".recomb", digits=6
    )
    InputOutput.writeIdnIndexedMatrix(
        pedigree, peelingInfo.recomb_mat, args.out + ".recomb_mat", digits=6
    )
    InputOutput.writeIdnIndexedMatrix(
        pedigree, peelingInfo.recomb_pat, args.out + ".recomb_pat", digits=6
    )
